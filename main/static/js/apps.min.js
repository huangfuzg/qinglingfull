		...
		$.getJSON("/static/parameter/"+pname+".json", "", function(data) {
       		...
       		for (var i = data.length - 1; i >= 0; i--) {
				...
				if(data[i].id === cata)
				{
					...
					console.log(data[i]);
					// if(data[i].doc!=null&&data[i].doc!=""){
					// 	console.log(data[i].doc);
					// 	var doc = data[i].doc;
					// 	var str = '<div class="corr">\
     //                      <a href="static/parameter/'+pname+'/'+doc+'" style="font-size:1.5em;">Product Doc</a>\
     //                      </div>';
     //                    $('#product-desc').append(str);
					// }
					if(data[i].drawing!=""&&data[i].drawing!=null){
						var str = '<div class="corr">\
                          <div class="image1">\
                              <img src="static/parameter/'+pname+'/'+data[i].drawing+'" alt=""  />\
                          </div>\
                          </div>';
                        $('#product-desc').append(str);
					}
